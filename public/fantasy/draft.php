...
$competition = CompetitionService::getCompetitionDetails($competitionId, $season);
$selections = FantasyService::getUserSelections($userId, $season, $competitionId);
$isLocked = FantasyService::isCompetitionLocked($season, $competitionId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$isLocked) {
	...
	if (!empty($athleteId) && in_array($action, ['add', 'remove'])) {
		$response = FantasyService::updateUserSelection($userId, $season, $competitionId, $athleteId, $action);
...
if (!empty($_GET['name']) || !empty($_GET['country'])) {
	...
	$searchResults = AthleteService::searchAthletes($searchName, $competition['gender'], $competition['weapon'], strtoupper(array_search($searchCountry, $validCountryCodes) ?: ''));
}
