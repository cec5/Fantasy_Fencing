...
<main class="container my-5">
		...
		<div class="col-md-3">
			...
			<select class="form-select" id="season" name="season">
				<?php foreach ($seasons as $seasonCode => $seasonName): ?>
					<option value="<?= $seasonCode ?>" <?= $seasonCode == $season ? 'selected' : '' ?>><?= $seasonName ?>
					</option>
				<?php endforeach; ?>
			</select>
		</div>
	...
	<div class="tab-content mt-3">
		...
		<div class="tab-pane fade show active" id="upcomingTab">
			<?php
			$upcoming = FantasyService::getUserCompetitions($userId, $season, false, $weapon, $gender, $ageCategory);
			if (empty($upcoming)) {
				echo "<p class='text-muted mt-3'>No upcoming competitions with your selections.</p>";
			} else {
				foreach ($upcoming as $comp):
					$sel = FantasyService::getUserSelections($userId, $season, $comp['competitionId']);
					?>
						...
						<table class="table table-striped table-bordered text-center">
							...
							<tbody>
								<tr>
									...
									<td><?= htmlspecialchars($competitionCategories[$comp['category']] ?? $comp['category']) ?>
									</td>
							...
							<tbody>
								<?php foreach ($sel as $i => $ath): ?>
									<tr>
										<td><?= $i + 1 ?></td>
										<td><a
												href="a/athlete.php?id=<?= $ath['athleteId'] ?>"><?= htmlspecialchars($ath['name']) ?></a>
										</td>
		...
		<div class="tab-pane fade" id="pastTab">
			<?php
			$past = FantasyService::getUserCompetitions($userId, $season, true, $weapon, $gender, $ageCategory);
			if (empty($past)) {
				echo "<p class='text-muted mt-3'>No past competitions with your selections.</p>";
			} else {
				foreach ($past as $comp):
					$sel = FantasyService::getUserSelections($userId, $season, $comp['competitionId']);
					$results = ResultService::getSpecificCompetitionResult($comp['competitionId'], $season);
					$pointsMap = [];
						...
						<table class="table table-striped table-bordered text-center">
							...
							<tbody>
								<tr>
									<td><?= htmlspecialchars($comp['startDate']) ?></td>
									<td><a
											href="c/competition.php?season=<?= $season ?>&id=<?= $comp['competitionId'] ?>"><?= htmlspecialchars($comp['name']) ?></a>
									</td>
									<td><?= htmlspecialchars($comp['location'] . ', ' . $comp['country']) ?></td>
