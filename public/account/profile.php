...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['updateUsername'])) {
		...
		$result = !empty($newUsername) ? AuthService::updateUsername($userId, $newUsername) : "Username cannot be empty.";
	} elseif (isset($_POST['updateEmail'])) {
		$newEmail = $_POST['email'];
		$result = filter_var($newEmail, FILTER_VALIDATE_EMAIL) ? AuthService::updateEmail($userId, $newEmail) : "Invalid email format.";
	} elseif (isset($_POST['updateNationality'])) {
		$newNationality = $_POST['countryCode'];
		$result = !empty($newNationality) ? AuthService::updateNationality($userId, $newNationality) : "Please select a valid nationality.";
	} elseif (isset($_POST['updatePassword'])) {
		$currentPassword = $_POST['currentPassword'];
		$newPassword = $_POST['newPassword'];
		$result = AuthService::updateUserPassword($userId, $currentPassword, $newPassword);
	}
