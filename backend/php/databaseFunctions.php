...
function updateAthleteSeasonPoints($season)
{
	...
	// Calculate total points per athlete, weapon, and age category for the given season
	$query = "
        	...
        	WHERE cr.season = ?
        	GROUP BY cr.athleteId, c.weapon, c.ageCategory
    	";
	$stmt = $db->prepare($query);
	...
	// Insert or update each athlete's total points by weapon and age category in athleteSeasonPoints
	$updateQuery = "
	...
	while ($row = $result->fetch_assoc()) {
		...
		$weapon = $row['weapon'];
		$ageCategory = $row['ageCategory']; // Make sure this is one of 'S', 'J', 'C', 'V'
...
// Get available age categories the athlete fenced in for a given season and weapon
function getAthleteAgeCategories($athleteId, $season, $weapon)
{
	$db = dbConnect();
	$query = "
        	SELECT DISTINCT c.ageCategory
        	FROM competitionResults cr
        	JOIN competitions c ON cr.competitionId = c.competitionId AND cr.season = c.season
...
function getTotalPoints($athleteId, $season, $weapon, $ageCategory)
{
	$db = dbConnect();
...
function getTopEarners($season, $weapon, $gender, $ageCategory)
{
	$db = dbConnect();
	...
	$query = "
        	...
        	WHERE asp.season = ? AND asp.weapon = ? AND a.gender = ? AND asp.ageCategory = ?
...
// Get list of competitions filtered by season, weapon, gender, and age category, ordered by date
function getCompetitions($season, $weapon, $gender, $ageCategory)
{
	$db = dbConnect();
...
function getCompetitionDetails($competitionId, $season)
{
	...
	$query = "
        	SELECT name, startDate, endDate, location, country, weapon, gender, category, ageCategory
        	FROM competitions 
